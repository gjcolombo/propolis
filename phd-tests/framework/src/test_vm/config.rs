...
use propolis_client::types::{
    Board, Chipset, ComponentV0, InstanceMetadata, InstanceSpecV0, NvmeDisk,
...
impl<'dr> VmConfig<'dr> {
    ...
    pub(crate) async fn vm_spec(
        ...
        for (idx, (req, hdl)) in all_disks.enumerate() {
            ...
            let _old = spec.components.insert(device_name, device_spec);
            assert!(_old.is_none());
            let _old = spec.components.insert(backend_name, backend_spec);
            assert!(_old.is_none());
        }
